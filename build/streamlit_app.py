...
with tabs[2]:
    ...
    if 'month' in df.columns and 'year' in df.columns:
        df_complete = df[df['year'] < 2025]  # drop the partial year
...
with tabs[3]:
    ...
    if 'dayofweek' in df.columns:
        dow = df.groupby('dayofweek').size().reset_index(name='incidents')
...
with tabs[4]:
    ...
    if 'hour' in df.columns:
        hour = df.groupby('hour').size().reset_index(name='incidents')
...
with tabs[5]:
    ...
    type_dist = df['nature'].value_counts().reset_index()
...
with tabs[7]:
    ...
    if 'nature_grp' in df.columns:
        grp = df['nature_grp'].value_counts().reset_index()
...
with tabs[8]:
    ...
    if 'hour' in df.columns and 'month' in df.columns:
        heatmap_data = df.groupby(['month','hour']).size().reset_index(name='incidents')
...
with tabs[9]:
    ...
    if 'lat' in df.columns and 'lon' in df.columns:
        fig = px.scatter_mapbox(df.dropna(subset=['lat','lon']), lat='lat', lon='lon', hover_data=['reported_dt','nature','address'],
